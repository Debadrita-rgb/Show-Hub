...
const serviceToEndpoint = {
  ...
  // Fitness: "my-fitness",
  // Salon: "my-salon",
  Stationary: "my-stationary",
  // Movie: "my-movie",
  // "Party Hall": "my-partyhall",
};

const Cart = () => {
  const location = useLocation();
  const queryParams = new URLSearchParams(location.search);
  const defaultTab = queryParams.get("tab") || "Catering";
  ...
  const handleTabChange = (tab) => {
    setSelectedTab(tab); // Update UI state
    navigate(`/cart?tab=${tab.toLowerCase()}`); // Update URL
    fetchCartItems(tab); // Fetch data for that tab
  };

  const fetchCartItems = async (tabName) => {
    try {
      const capitalizedTab = tabName.charAt(0).toUpperCase() + tabName.slice(1).toLowerCase();
      const endpoint = serviceToEndpoint[capitalizedTab];

      if (!endpoint) {
        console.error(`No endpoint found for tab: ${capitalizedTab}`);
        return;
      }
  ...
  useEffect(() => {
    const tabFromUrl = queryParams.get("tab") || "Catering";
    const capitalizedTab =
      tabFromUrl.charAt(0).toUpperCase() + tabFromUrl.slice(1).toLowerCase();
    setSelectedTab(capitalizedTab);
    fetchCartItems(capitalizedTab);
  }, [location.search]);
  ...
  const updateQuantity = async (itemId, delta) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.patch(
        "http://localhost:5000/voyager/update-catering-quantity",
        {
          orderId: item.orderId, // orderId was added to items from backend
          itemId: item._id,
  ...
  const deleteItem = async (orderId, itemId) => {
    try {
      ...
      const res = await axios.delete(
        "http://localhost:5000/voyager/delete-item-catering",
        {
          data: { orderId, itemId },
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      // After deleting, update the cart items in the state
  ...
  const updateItemStatus = async (orderId, newStatus) => {
    try {
      ...
      const token = localStorage.getItem("token");
      await axios.put(
        `http://localhost:5000/voyager/update-status-${endpoint.split("-")[1]}`, // extracts 'catering' or 'stationary'
        {
          orderId,
          newStatus,
        },
      ...
      setTimeout(() => {
        navigate(`/cart?tab=${selectedTab.toLowerCase()}`);
      }, 2000);
          ...
          <div className="mx-auto mt-6 max-w-4xl flex-1 space-y-6 lg:mt-0 lg:w-full">
            <div className="space-y-4 rounded-lg border border-gray-200 p-4 bg-gray-200 sm:p-6">
              ...
              {cartItems.length > 0 && (
                <button
                  onClick={() =>
                    updateItemStatus(cartItems[0].orderId, "Ordered")
                  }
